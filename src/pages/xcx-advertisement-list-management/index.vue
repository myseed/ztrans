<template>
  <d2-container>
    ...
    <el-dialog title="增加小程序首页广告" :visible.sync="addAdverside">
...
import {
    getXcxFirstPageList,addXcxFirstPageList
} from "@/api/advertise";
...
export default {
  ...
  methods: {
    ...
    handleSubmit(form) {
      ...
      addConfirm(){
        this._addXcxFirstPageList({
            bizContentName:this.url
        });
      },
      _addXcxFirstPageList(param){
          addXcxFirstPageList({
              customerNumId: util.cookies.get('__user__customernumid'),
              bizContentName:param.bizContentName,
          })
              .then(res => {
                  if(res.code==0){
                      this.$message({
